...
steps {
          ...
          slackSend (color: "add8e6", message: 'salesforcecreate-mule3 Deployment Started')
          buildsrc() 
          slackSend (color: "0000ff", message: 'salesforcecreate-mule3 Build Sucessfully')
...
stage('upload to atifactory') {
  ...
  "files": [
    {
      "pattern": "**/*.jar",
      "target": "generic-local/mule3-demo-jars_$BUILD_NUMBER/salesforcecreate-mule3.jar"
    }
 ]
}"""                 
              def buildInfo1 = server.upload spec: uploadSpec
                      slackSend (color: "#FFA500",message: 'salesforcecreate-mule3 Artifacts Uploaded Sucessfully')
            }
    }
}  	
}
   post {
      failure {
           ...
           slackSend (color: "#FF0000",message: 'salesforcecreate-mule3 Deployment Failed')
        }
      success {
...
''', subject: 'Jenkins ${BUILD_STATUS} [#${BUILD_NUMBER}] - ${PROJECT_NAME} ${ENV, var="GIT_URL"}', to: 'devops@eaiesb.com'    
          slackSend (color: "#32CD32", message: 'salesforcecreate-mule3 Deployment is Sucessful')
        }
  }
}
// steps
